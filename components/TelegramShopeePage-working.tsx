                ...
                try {
                    ...
                    if (chatResult.ok) {
                        ...
                        switch (chatInfo.type) {
                            case 'private': chatType = 'Chat Privado'; break;
                            case 'group': chatType = 'Grupo'; break;
                            case 'supergroup': chatType = 'Supergrupo'; break;
                            case 'channel': chatType = 'Canal'; break;
                            default: chatType = chatInfo.type;
                        }
                        
                        testResults.push(`✅ ${destination.name}\n   Tipo: ${chatType}\n   ID: ${chatInfo.id}`);
                    } else {
                        testResults.push(`❌ ${destination.name}\n   Erro: ${chatResult.description}`);
                    }
                } catch (err) {
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="mt-2 p-2 bg-blue-900/30 border border-blue-700 rounded text-xs text-blue-300">
                        ...
                        2. Clique em um chat para copiar o ID<br/>
                    ...
                    {showChatList && foundChats.length > 0 && (
                        <div className="mt-4 p-4 bg-slate-800/50 border border-dark-border rounded-lg">
                            <h4 className="text-sm font-semibold text-dark-text-primary mb-3">
                                📋 Chats encontrados - Clique para copiar o ID:
                            </h4>
                            <div className="space-y-2 max-h-40 overflow-y-auto">
                                {foundChats.map((chat) => {
                                    ...
                                    const isMigrated = chat.migratedFrom && chat.migratedFrom.toString() === config.chatId;
                                    
                                    return (
                                        <button
                                            key={chat.id}
                                            onClick={() => copyToClipboard(chat.id.toString(), chat.title)}
                                            className={`w-full text-left p-3 hover:bg-slate-700 border rounded-lg transition-colors group ${
                                                isMigrated 
